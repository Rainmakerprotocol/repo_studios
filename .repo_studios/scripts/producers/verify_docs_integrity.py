...
def _process_documents(
    docs: list[dict[str, Any]], update: bool, repo_root: Path
...
def verify_all(paths: Paths, options: Options) -> VerificationOutcome:
    ...
    try:
        index_results, index_blocks, index_changed = process_file(
            paths.index_path, update=options.update
        )
    except Exception as exc:  # pragma: no cover - defensive
...
def render_markdown_report(payload: dict[str, Any]) -> str:
    ...
    if payload.get('status') == 'mismatches':
        ...
        for item in payload.get('mismatches', []):
            lines.append(
                f"| `{item['path']}` | {item['block_index']} | `{item['computed_hash']}` |\n"
            )
...
def render_log(payload: dict[str, Any]) -> str:
    ...
    if payload.get('missing_documents'):
        entries.append(
            'missing_documents=' + ','.join(str(item) for item in payload['missing_documents'])
        )
    if payload.get('errors'):

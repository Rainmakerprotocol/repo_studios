...
def extract_rules(
    ...
    marker_rules, marker_dupes, marker_conflicts, marker_invalid_severity = _extract_marker_blocks(
        text, categories, existing_ids, rel_file, today_str
    )

    heading_rules, heading_conflicts, heading_invalid_severity = _extract_heading_rules(
    ...
    invalid_severity_rules = sorted(marker_invalid_severity | heading_invalid_severity)
    ...
    diagnostics = {
        ...
        "invalid_severity_rules": invalid_severity_rules,
        "notes": [
...
def _extract_marker_blocks(
    ...
    rel_file: str,
    today: str,
) -> tuple[list[ParsedRule], set[str], set[str], set[str]]:
    rules: list[ParsedRule] = []
    duplicates: set[str] = set()
    conflicts: set[str] = set()
    invalid_severity: set[str] = set()
    ...
    while i < len(lines):
        ...
        if line.startswith("<!-- standards:rule"):
            ...
            if any(r.id == rid for r in rules):
                duplicates.add(rid)
                continue
            cat_ids = _split_multi(meta["categories"]) if meta.get("categories") else list(categories)
            applies_raw = meta["applies_to"]
            applies = _split_multi(applies_raw)
            if not applies and applies_raw.strip():
                applies = [applies_raw.strip()]
            severity = _normalize_severity(meta["severity"], rid, invalid_severity)
            if severity is None:
                continue
            rule = ParsedRule(
                id=rid,
            ...
            i += 1
    return rules, duplicates, conflicts, invalid_severity
    ...
    while i < len(lines):
        ...
        if all(bullet_map.values()):
            ...
            if rid in existing_ids:
                ...
                if not applies and applies_raw.strip():
                    applies = [applies_raw.strip()]
                cat_ids = _split_multi(bullets["categories"]) if bullets.get("categories") else list(categories)
                rules.append(
                    ParsedRule(
                        id=rid,
                        category_ids=cat_ids or list(categories),
        ...
        i += 1
    return rules, conflicts, invalid_severity
...
def _normalize_severity(raw: str, rid: str, invalid: set[str]) -> str | None:
    value = raw.strip().lower()
    normalized = _SEVERITY_MAP.get(value)

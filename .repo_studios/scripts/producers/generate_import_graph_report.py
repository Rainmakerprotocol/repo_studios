...
def write_artifacts(
    ...
    *,
    ...
    generated_ts = datetime.fromisoformat(report["generated_utc"]).astimezone(timezone.utc)
    run_dir = output_dir / f"{RUN_PREFIX}-{generated_ts.strftime('%Y%m%d_%H%M%S')}"
    run_dir.mkdir(parents=True, exist_ok=True)

    graph_path = run_dir / "graph.json"
    graph_path.write_text(
        json.dumps(_serialize_graph(graph), indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )

    report_with_paths = json.loads(json.dumps(report))
    report_with_paths["graph_path"] = str(graph_path)
    report_with_paths["run_directory"] = str(run_dir)
    ...
    md_path.write_text(write_markdown(report_with_paths), encoding="utf-8")

    log_path = run_dir / "log.txt"
    log_path.write_text(write_log(report_with_paths), encoding="utf-8")

    latest_pairs = [
        (json_path, output_dir / "latest_report.json"),
        (md_path, output_dir / "latest_report.md"),
        (log_path, output_dir / "latest_report.log"),
        (graph_path, output_dir / "latest_graph.json"),
    ]

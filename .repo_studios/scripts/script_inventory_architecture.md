...
| collect_faulthandler_reports.py | DONE | DONE | TODO | Structured faulthandler bundle emits JSON/MD/CSV alongside combined logs with pruning and latest pointers; automation doc and Make target pending orchestrator alignment. |
| collect_test_log_reports.py | TODO | TODO | TODO | Blueprint drafted (`command_center/docs/phase_7/COLLECT_TEST_LOG_REPORTS_BLUEPRINT.md`) to wrap `utilities.test_log_analysis`; implementation pending. |
...
**Status Update (2025-11-23):** Completed the producer testing reference audit, documentation refresh (validate_markdown_anchors), extraction consumer audit, placeholder remediation plan drafting (see `.repo_studios/command_center/docs/phase_7/PLACEHOLDER_DEBT_PLAN.md`), top offender cataloging, owner outreach + allowlist guidance publication, exclusion + uppercase heuristics rollout with matching docs/tests, the repo-root scan/ledger update capturing a zero-match baseline, CI gating proposal draft, and the warning-mode `placeholder-scan` workflow stub; newly mirrored monkey-patch artifacts into the legacy alias so consumers stay unblocked during the schema transition and taught the anchor health consumer to reuse anchor inventory artifacts (with inventory vs scan telemetry). Next up is sustaining weekly scans, monitoring the workflow + rollout log, aligning stakeholders on the enforcement timeline, and executing the remaining consumer follow-ups.

**Status Update (2025-11-23 follow-up):** Past — Verified the faulthandler consumer now prefers the structured producer bundle, refreshed the inventory tables, and published the pytest log producer blueprint. Present — Coordinating the faulthandler automation doc/Make wiring and preparing test fixtures for the upcoming producer implementation. Future — Will implement the pytest log producer, publish the faulthandler automation doc/Make target, and continue the monkey-patch consumer refactor.
...
| Script Path | Description | Needed | Wired | Dependencies | Testing Coverage | Entry Points | Migration Target | Conformance Review | Duplicates | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| .repo_studios/scripts/consumers/classify_monkey_patches.py | Buckets monkey-patch findings into HIGH/MODERATE/SAFE risk and writes JSON/MD summaries. | yes | manual CLI; orchestrator | `scan_monkey_patches.py` runs (`.repo_studios/reports/producer_reports/monkey_patch_scans/*` mirrored via `.repo_studios/monkey_patch/*`) | TODO | manual; orchestrate_health_suite | scripts/consumers/classify_monkey_patches.py | TODO | none | Emits `RISK_SUMMARY.(json & md)` beside latest scan; legacy alias now copies matches → report.json plus summary.json for compatibility. |
...
| .repo_studios/scripts/consumers/generate_test_log_health_report.py | Summarizes pytest logs for warnings, slow tests, and JUnit stats. | yes | manual CLI (--logs-dir/--output-base) | pytest log bundles (`.repo_studios/pytest_logs/*` from orchestrators/run_pytest_log_capture.py); defusedxml | TODO | manual; orchestrate_health_suite | scripts/consumers/generate_test_log_health_report.py | TODO | none | Writes report JSON/MD under `.repo_studios/test_health/<ts>/`; relies on orchestrator to stage inputs; blueprint for upstream producer lives at `command_center/docs/phase_7/COLLECT_TEST_LOG_REPORTS_BLUEPRINT.md`. |
...
- [ ] Publish automation doc and Make target for `collect_faulthandler_reports.py` once orchestrator wiring reuses the structured bundle.
...
- [ ] Update monkey-patch consumer to ingest summary + matches schema without relying on the alias after refactor loop hardening.

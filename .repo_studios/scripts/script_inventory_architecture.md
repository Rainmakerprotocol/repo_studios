...
| .repo_studios/scripts/orchestrators/run_fault_pipeline.py | Orchestrates the faulthandler producer → consumer chain and mirrors summaries into command center. | yes | manual CLI (`--repo-root/--skip-producer/--skip-consumer/--reuse-report`); Make target TBD | `collect_faulthandler_reports.py`; `generate_fault_artifacts.py`; command_center libraries (`artifacts`, `cli`) | tests/tests_orchestrators/test_run_fault_pipeline.py | manual | scripts/orchestrators/run_fault_pipeline.py | std-global-python-engineering.md | none | Writes JSON/Markdown/log bundles under `.repo_studios/reports/orchestrator_runs/fault_pipeline/`, maintains `latest_*` pointers, mirrors bundles to `.repo_studios/command_center/reports/fault_pipeline_orchestrator/`, honors retention flags, and supports reuse of producer reports; downstream aggregator design still pending. |
...
- [ ] Draft faulthandler aggregator blueprint once the orchestrator stabilizes; record scope under `.repo_studios/docs/automation/traceability/fault_pipeline_aggregator_plan.md`.

...
| .repo_studios/scripts/utilities/test_log_analysis.py | Parses pytest logs and JUnit XML into structured health reports plus Markdown summaries. | yes | library (imported) | defusedxml (optional); stdlib | tests/tests_utilities/test_test_log_analysis.py | library import | scripts/utilities/test_log_analysis.py | std-global-python-engineering.md | none | Supplies log selection, warning aggregation, slow-test extraction, and markdown rendering for the pytest log producer/consumer chain. |

...
| .repo_studios/scripts/producers/generate_undocumented_logic_report.py | Highlights modules/functions that should have documentation anchors but do not. | planned | TODO | anchor inventory, doc index owners, code annotations | TODO | TODO | scripts/producers/generate_undocumented_logic_report.py | std-global-python-engineering.md | none | TODO: define heuristics, structure outputs, add pytest coverage, publish automation doc, wire retention controls. |
...
- [ ] Continue consumer refactors before touching aggregators; queue follow-up work once the consumer row TODOs are cleared.
...
| .repo_studios/scripts/aggregators/aggregate_docs_health_signals.py | Combines churn warnings, undocumented logic findings, and anchor duplicates into a unified docs health dashboard. | planned | TODO | `generate_code_doc_churn_report.py`, `generate_undocumented_logic_report.py`, anchor inventory/health bundles | TODO | TODO | scripts/aggregators/aggregate_docs_health_signals.py | std-global-markdown-authoring.md | none | TODO: sequence producer inputs, emit JSON/MD summary with provenance, add pytest coverage, publish automation doc. |

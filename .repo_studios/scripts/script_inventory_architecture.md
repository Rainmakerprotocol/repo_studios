...
role: [ArchitectureDoc]
owners: [repo_studios_ai]
status: draft
version: 1
updated_at: 2025-12-17
tags: [inventory, scripts, architecture]
related_files:
...
| check_inventory_health.py | DONE | DONE | DONE | Positional artifacts (`manifest.json`, `summary.md`, `telemetry.json`) under `.repo_studios/command_center/reports/healthview/inventory_health/<YYYYMMDD-HHMM>/` with pruning, DB integration markers + dual-write storage, pytest `tests/tests_producers/test_check_inventory_health.py`, Make target `studio-check-inventory-health`, automation doc at `docs/automation/check_inventory_health.md`, DB integration doc at `.repo_studios/command_center/docs/db_integrations/db_integration_check_inventory_health.md`. |
| diff_standards_index.py | DONE | DONE | DONE | Canonical bundle emitter under `.repo_studios/command_center/reports/rawview/standards_index_diff/<YYYYMMDD-HHMM>/` with pruning; DB integration markers present for manifest/summary/telemetry writes; automation doc updated. |
...
| generate_doc_index.py | DONE | DONE | DONE | Repo-wide documentation index now emits a positional canonical bundle (`manifest.json`, `summary.md`, `telemetry.json`) under `.repo_studios/reports/producer_reports/healthview/doc_index/<YYYYMMDD-HHMM>/` with retention=1, shared storage + pruning, DB integration markers at write sites, and database sink placeholder logging. |
| generate_undocumented_logic_report.py | DONE | DONE | DONE | Positional canonical bundle (`manifest.json`, `summary.md`, `telemetry.json`) under `.repo_studios/reports/producer_reports/healthview/undocumented_logic/<YYYYMMDD-HHMM>/` with retention pruning; storage writes include DB integration markers + dual-write storage; no `latest_*` pointers; pytest coverage and Make target `studio-generate-undocumented-logic-report`; automation doc at `.repo_studios/docs/automation/generate_undocumented_logic_report.md`; DB integration doc at `.repo_studios/command_center/docs/db_integrations/db_integration_undocumented_logic.md`. |
| generate_dependency_hygiene_report.py | DONE | DONE | DONE | Positional canonical bundle (`manifest.json`, `summary.md`, `telemetry.json`) under `.repo_studios/reports/producer_reports/healthview/dependency_hygiene/<YYYYMMDD-HHMM>/` with retention pruning; storage writes include DB integration markers + dual-write storage; no `latest_*` pointers; pytest coverage, Make target `studio-generate-dependency-hygiene`; automation doc at `.repo_studios/docs/automation/generate_dependency_hygiene_report.md`; DB integration doc at `.repo_studios/command_center/docs/db_integrations/dependency_hygiene.md`. |
| collect_faulthandler_reports.py | DONE | DONE | DONE | Positional artifacts (`manifest.json`, `summary.md`, `telemetry.json`) under `.repo_studios/command_center/reports/rawview/fault_artifacts_producer/<YYYYMMDD-HHMM>/` with pruning, DB integration markers + dual-write storage, pytest `tests/tests_producers/test_collect_faulthandler_reports.py`, automation doc (`docs/automation/collect_faulthandler_reports.md`), and Make target `studio-collect-faulthandler-reports`. |
...
| generate_import_graph_report.py | DONE | DONE | DONE | Positional canonical bundle (`manifest.json`, `summary.md`, `telemetry.json`) under `.repo_studios/reports/producer_reports/healthview/import_graph/<YYYYMMDD-HHMM>/` with retention pruning; storage writes include DB integration markers + dual-write storage; no `latest_*` pointers; pytest coverage and Make target `studio-generate-import-graph`; automation doc at `.repo_studios/docs/automation/generate_import_graph_report.md`; DB integration doc at `.repo_studios/command_center/docs/db_integrations/db_integration_import_graph.md`. |
| generate_lizard_report.py | DONE | DONE | DONE | Positional canonical bundle (`manifest.json`, `summary.md`, `telemetry.json`) under `.repo_studios/reports/producer_reports/healthview/lizard_report/<YYYYMMDD-HHMM>/` with retention pruning; storage writes include DB integration markers + dual-write storage; no `latest_*` pointers; pytest coverage and Make target `studio-generate-lizard-report`; automation doc at `.repo_studios/docs/automation/generate_lizard_report.md`; DB integration doc at `.repo_studios/command_center/docs/db_integrations/db_integration_lizard_report.md`. |
...
| generate_typecheck_report.py | DONE | DONE | DONE | Emits canonical bundles under `.repo_studios/reports/producer_reports/healthview/typecheck_report/<YYYYMMDD-HHMM>/` (manifest/summary/telemetry only) with pruning and DB integration markers. |
| generate_test_coverage_inventory.py | DONE | DONE | DONE | Coverage XML ingester emits positional bundles (`manifest.json`, `summary.md`, `telemetry.json`) under `.repo_studios/reports/producer_reports/healthview/test_coverage_inventory/<YYYYMMDD-HHMM>/` with pruning, DB integration markers + dual-write storage, pytest coverage, Make target `studio-generate-test-coverage-inventory`, and automation doc dated 2025-12-17. |
| analyze_test_hardening.py | DONE | DONE | DONE | Positional artifacts (`manifest.json`, `summary.md`, `telemetry.json`) under `.repo_studios/command_center/reports/healthview/test_hardening/<YYYYMMDD-HHMM>/` with pruning, DB integration markers + dual-write storage, pytest `tests/tests_producers/test_analyze_test_hardening.py`, Make target `studio-analyze-test-hardening`, automation doc at `docs/automation/analyze_test_hardening.md`, DB integration doc at `.repo_studios/command_center/docs/db_integrations/db_integration_analyze_test_hardening.md`. |
| render_inventory_views.py | DONE | DONE | DONE | Positional canonical bundle (`manifest.json`, `summary.md`, `telemetry.json`) under `.repo_studios/reports/producer_reports/healthview/inventory_overview/<YYYYMMDD-HHMM>/` with overwrite pruning (keep=1), storage dual-write + DB markers, and refreshed legacy stubs (no `latest_*`). |
...
| scan_monkey_patches.py | DONE | DONE | DONE | Positional canonical bundle (`manifest.json`, `summary.md`, `telemetry.json`) under `.repo_studios/reports/producer_reports/healthview/monkey_patches/<YYYYMMDD-HHMM>/` with pruning and DB integration markers; no `latest_*` pointers; pytest `tests/tests_producers/test_scan_monkey_patches.py`, Make target `studio-scan-monkey-patches`, automation doc `.repo_studios/docs/automation/scan_monkey_patches.md`, DB integration doc `.repo_studios/command_center/docs/db_integrations/db_integration_monkey_patches.md`. |
...
| generate_code_doc_churn_report.py | DONE | DONE | DONE | Positional canonical bundle (`manifest.json`, `summary.md`, `telemetry.json`) under `.repo_studios/reports/producer_reports/healthview/code_doc_churn/<YYYYMMDD-HHMM>/` with retention pruning; storage writes include DB integration markers + dual-write storage; no `latest_*` pointers; pytest coverage and Make target `studio-generate-code-doc-churn-report`; automation doc at `.repo_studios/docs/automation/generate_code_doc_churn_report.md`; DB integration doc at `.repo_studios/command_center/docs/db_integrations/code_doc_churn.md`. |
...
| .repo_studios/scripts/producers/diff_standards_index.py | Diffs two standards index YAML snapshots and classifies rule-level changes. | yes | manual CLI (`--fail-on`); make studio-diff-standards-index (`old=<path>` `new=<path>`; override `FAIL_ON` for smoke runs) | PyYAML; command_center storage + pruning libraries | tests/tests_producers/test_diff_standards_index.py | manual; Make target | scripts/producers/diff_standards_index.py | std-global-python-engineering.md; REPORT_NAMING_STANDARDS.md | none | Emits canonical bundle artifacts (`manifest.json`, `summary.md`, `telemetry.json`) under `.repo_studios/command_center/reports/rawview/standards_index_diff/<YYYYMMDD-HHMM>/` with pruning and DB integration markers at write sites. Make target runs from repo root and resolves `old/new` paths consistently. Catalog: scripts.standards.index_diff. |
...
| .repo_studios/scripts/producers/generate_dependency_hygiene_report.py | Audits requirements and pyproject deps for pins, VCS refs, and duplicates. | yes | manual CLI (--repo-root/--output-dir); make studio-generate-dependency-hygiene | tomllib; requirements*.txt; pyproject.toml | tests/tests_producers/test_generate_dependency_hygiene_report.py | manual; Make target | scripts/producers/generate_dependency_hygiene_report.py | std-global-python-engineering.md | none | Emits positional canonical bundle artifacts (`manifest.json`, `summary.md`, `telemetry.json`) under `.repo_studios/reports/producer_reports/healthview/dependency_hygiene/<YYYYMMDD-HHMM>/` with pruning; storage writes include DB integration markers; `telemetry.json` retains the legacy payload under `payload` for agent drill-down. Automation doc at `.repo_studios/docs/automation/generate_dependency_hygiene_report.md`; DB integration doc at `.repo_studios/command_center/docs/db_integrations/dependency_hygiene.md`. Catalog: scripts.health.dep_hygiene_report. |
| .repo_studios/scripts/producers/collect_faulthandler_reports.py | Converts faulthandler run directories into positional-encoded canonical artifacts (manifest/summary/telemetry) with pruning and DB integration markers; no `latest_*` pointers. | yes | manual CLI (--runs-dir/--output-dir); make studio-collect-faulthandler-reports | `.repo_studios/command_center/reports/rawview/fault_diagnostics_runs/<ts>/` runs (optional legacy fallback `.repo_studios/faulthandler/`); `utilities.fault_run_analysis`; command_center storage/prune libs | tests/tests_producers/test_collect_faulthandler_reports.py | manual; orchestrate_health_suite | scripts/producers/collect_faulthandler_reports.py | TODO | none | Automation doc `docs/automation/collect_faulthandler_reports.md` refreshed; Make target writes under `.repo_studios/command_center/reports/rawview/fault_artifacts_producer/<YYYYMMDD-HHMM>/`. Catalog: scripts.health.collect_faulthandler_reports. |
| .repo_studios/scripts/producers/collect_test_log_reports.py | Collects pytest log runs into structured JSON/MD/CSV/log bundles with retention pruning and latest pointers. | yes | manual CLI (--logs-dir/--logs-run); make studio-collect-test-log-reports; orchestrate_health_suite | command_center.scripts.libraries.test_log_analysis; command_center.scripts.libraries.prune_run_directories; `.repo_studios/command_center/reports/rawview/test_execution_runs` | tests/tests_producers/test_collect_test_log_reports.py | manual; orchestrate_health_suite; run_pytest_log_capture | scripts/producers/collect_test_log_reports.py | std-global-python-engineering.md | none | Emits run bundles under `.repo_studios/reports/producer_reports/test_log_reports/` with latest hardlinks, legacy pytest log fallback, combined log copies, and summary metrics for downstream consumers. |
| .repo_studios/scripts/producers/generate_import_graph_report.py | Builds adjacency graph of owned packages, surfacing cycles and fan-in/out hotspots. | yes | manual CLI (--owned); make studio-generate-import-graph | python stdlib; repo source tree; command_center storage/prune libs | tests/tests_producers/test_generate_import_graph_report.py | manual; orchestrators | scripts/producers/generate_import_graph_report.py | std-global-python-engineering.md | none | Emits positional canonical bundle artifacts (`manifest.json`, `summary.md`, `telemetry.json`) under `.repo_studios/reports/producer_reports/healthview/import_graph/<YYYYMMDD-HHMM>/` with pruning; storage writes include DB integration markers; no `latest_*` pointers. Automation doc at `.repo_studios/docs/automation/generate_import_graph_report.md`; DB integration doc at `.repo_studios/command_center/docs/db_integrations/db_integration_import_graph.md`. Catalog: scripts.health.import_graph_report. |
| .repo_studios/scripts/producers/generate_lizard_report.py | Runs `python -m lizard` with repo thresholds and records offender summaries. | yes | manual CLI (--targets/--timestamp); make studio-generate-lizard-report | lizard module; repo source tree; command_center storage/prune libs | tests/tests_producers/test_generate_lizard_report.py | manual; Make target | scripts/producers/generate_lizard_report.py | TODO | none | Emits positional canonical bundle artifacts (`manifest.json`, `summary.md`, `telemetry.json`) under `.repo_studios/reports/producer_reports/healthview/lizard_report/<YYYYMMDD-HHMM>/` with pruning; storage writes include DB integration markers; no `latest_*` pointers. Automation doc at `.repo_studios/docs/automation/generate_lizard_report.md`; DB integration doc at `.repo_studios/command_center/docs/db_integrations/db_integration_lizard_report.md`. Catalog: scripts.health.lizard_report. |
| .repo_studios/scripts/producers/generate_typecheck_report.py | Drives mypy runs with repo configs and captures failing snapshots. | yes | `make -C .repo_studios typecheck` (or `make -C .repo_studios studio-generate-typecheck-report`) | mypy; pyproject settings; env toggles | tests/tests_producers/test_generate_typecheck_report.py | manual; orchestrate_health_suite | scripts/producers/generate_typecheck_report.py | std-global-python-engineering.md | none | Emits canonical bundles under `.repo_studios/reports/producer_reports/healthview/typecheck_report/<YYYYMMDD-HHMM>/` containing `manifest.json`, `summary.md`, and `telemetry.json` (no `latest_*`). Doc: `.repo_studios/docs/automation/generate_typecheck_report.md`. Catalog: scripts.health.typecheck_report. |
| .repo_studios/scripts/producers/generate_test_coverage_inventory.py | Summarizes Coverage.py XML into per-file function coverage metrics. | yes | make studio-generate-test-coverage-inventory; manual CLI (--coverage-xml) | Coverage.py XML report; repo source tree; `ast` | tests/tests_producers/test_generate_test_coverage_inventory.py | manual | scripts/producers/generate_test_coverage_inventory.py | std-global-python-engineering.md | none | Emits positional bundle artifacts (`manifest.json`, `summary.md`, `telemetry.json`) under `.repo_studios/reports/producer_reports/healthview/test_coverage_inventory/<YYYYMMDD-HHMM>/` with pruning, DB integration markers + dual-write storage, and no `latest_*` pointers. Automation doc at `.repo_studios/docs/automation/generate_test_coverage_inventory.md`; DB integration doc at `.repo_studios/command_center/docs/db_integrations/db_integration_generate_test_coverage_inventory.md`. Catalog: scripts.health.test_coverage_inventory. |
| .repo_studios/scripts/producers/analyze_test_hardening.py | Evaluates test modules for hardening gaps (missing asserts, mocks, long tests). | yes | CLI (--repo-root); make studio-analyze-test-hardening; GitHub Actions `studio-inventory` workflow | stdlib ast/json/re; repo tests tree | tests/tests_producers/test_analyze_test_hardening.py | manual; Make target; CI | scripts/producers/analyze_test_hardening.py | std-global-python-engineering.md | none | Emits positional-encoded artifacts under `.repo_studios/command_center/reports/healthview/test_hardening/<YYYYMMDD-HHMM>/` (`manifest.json`, `summary.md`, `telemetry.json`) with pruning; storage writes include DB integration markers; automation doc: `docs/automation/analyze_test_hardening.md`; DB integration doc: `.repo_studios/command_center/docs/db_integrations/db_integration_analyze_test_hardening.md`. Catalog: scripts.health.analyze_test_hardening. |
| .repo_studios/scripts/producers/scan_code_placeholders.py | Greps source files for placeholder comments like TODO/FIXME to surface debt hotspots. | yes | manual CLI (`--repo-root/--root` options); make studio-scan-code-placeholders | stdlib `argparse`, `pathlib`, `re` + command_center storage/prune libs | tests/tests_producers/test_scan_code_placeholders.py | manual | scripts/producers/scan_code_placeholders.py | std-global-python-engineering.md | none | Emits canonical bundle artifacts (`manifest.json`, `summary.md`, `telemetry.json`) under `.repo_studios/reports/producer_reports/healthview/code_placeholders/<YYYYMMDD-HHMM>/` with pruning and DB integration markers at write sites; no `latest_*` pointers. Automation doc at `.repo_studios/docs/automation/scan_code_placeholders.md`. Catalog: scripts.utilities.find_placeholders. |
| .repo_studios/scripts/producers/scan_monkey_patches.py | AST + regex inventory of monkey patches with positional-encoded canonical bundle outputs and telemetry extraction. | yes | make studio-scan-monkey-patches; manual CLI (--repo-root) | stdlib; optional git blame | tests/tests_producers/test_scan_monkey_patches.py | manual | scripts/producers/scan_monkey_patches.py | std-global-python-engineering.md | none | Updated 2025-12-17: emits positional canonical bundle artifacts (`manifest.json`, `summary.md`, `telemetry.json`) under `.repo_studios/reports/producer_reports/healthview/monkey_patches/<YYYYMMDD-HHMM>/` with pruning; storage writes include DB integration markers + dual-write storage; no `latest_*` pointers or legacy alias directories. Automation doc `.repo_studios/docs/automation/scan_monkey_patches.md`; DB integration doc `.repo_studios/command_center/docs/db_integrations/db_integration_monkey_patches.md`. Catalog: scripts.health.scan_monkey_patches. |
| .repo_studios/scripts/producers/validate_import_boundaries.py | Enforces layering rules using import graph snapshots and static scans. | yes | manual CLI (--repo-root); legacy alias `check_import_boundaries.py` | `.repo_studios/reports/producer_reports/healthview/import_graph/*/telemetry.json` (payload.graph); `import_rules_allowlist.json`; repo source tree | tests/tests_producers/test_validate_import_boundaries.py | manual; make studio-validate-import-boundaries | scripts/producers/validate_import_boundaries.py | std-global-python-engineering.md | none | Emits structured JSON/MD/log/violations bundle with pruning, latest pointers, pytest coverage, and automation doc (`docs/automation/validate_import_boundaries.md`). Defaults to the latest healthview import graph bundle for graph input. Catalog: scripts.utilities.check_import_boundaries. |
...
| .repo_studios/scripts/producers/generate_doc_index.py | Builds repo-wide documentation index bundle with metrics/advisories plus JSON/YAML/CSV views and optional database sink placeholder metadata. | yes | manual CLI (--repo-root/--output-dir/--db-target/--log-level) | stdlib; PyYAML; command_center storage + pruning libraries | tests/tests_producers/test_generate_doc_index.py | manual | scripts/producers/generate_doc_index.py | std-global-markdown-authoring.md | none | Emits positional canonical bundle artifacts (`manifest.json`, `summary.md`, `telemetry.json`) under `.repo_studios/reports/producer_reports/healthview/doc_index/<YYYYMMDD-HHMM>/` with retention pruning; no `latest_*` pointers; `telemetry.json` retains the legacy payload under `payload` for agent drill-down; `summary.md` embeds JSON/YAML/CSV views for human review. Automation doc at `.repo_studios/docs/automation/generate_doc_index.md`. Catalog: scripts.docs.generate_doc_index. |
| .repo_studios/scripts/producers/generate_code_doc_churn_report.py | Flags source directories changed within a git window that lack matching docs updates. | yes | manual CLI (--repo-root/--git-window); make studio-generate-code-doc-churn-report | git CLI; doc index telemetry payload under `.repo_studios/reports/producer_reports/healthview/doc_index/` (defaults to latest run); `.repo_studios/reports/producer_reports/healthview/anchor_inventory/` enrichment; command_center storage + pruning libraries | tests/tests_producers/test_generate_code_doc_churn_report.py | manual; Make target | scripts/producers/generate_code_doc_churn_report.py | std-global-python-engineering.md | none | Emits positional canonical bundle artifacts (`manifest.json`, `summary.md`, `telemetry.json`) under `.repo_studios/reports/producer_reports/healthview/code_doc_churn/<YYYYMMDD-HHMM>/` with pruning; storage writes include DB integration markers; `telemetry.json` retains the legacy payload under `payload` for agent drill-down. Automation doc at `.repo_studios/docs/automation/generate_code_doc_churn_report.md`; DB integration doc at `.repo_studios/command_center/docs/db_integrations/code_doc_churn.md`. Catalog: scripts.docs.code_doc_churn_report. |
...
| .repo_studios/scripts/consumers/generate_anchor_health_report.py | Captures duplicate H1/H2 anchor slugs across docs tree and writes JSON/MD snapshots. | yes | manual CLI | `generate_anchor_inventory.py` outputs (canonical topic dir `.repo_studios/reports/producer_reports/healthview/anchor_inventory/` via loader compatibility) plus docs baseline (`tests/docs/anchor_slug_baseline.json`) | tests/tests_consumers/test_generate_anchor_health_report.py | manual | scripts/consumers/generate_anchor_health_report.py | std-global-markdown-authoring.md | none | Structured bundles now emit `summary.json`, `SUMMARY.md`, and `bundle_summary.json` alongside legacy `anchor_report.*` artifacts with latest pointers, retention=5, inventory fallback, database placeholder stub, and automation doc at `.repo_studios/docs/automation/generate_anchor_health_report.md`. Catalog: scripts.health.anchor_health_report. |
| .repo_studios/scripts/consumers/generate_fault_artifacts.py | Converts faulthandler stacks into CSV/MD/manifest artifacts for triage. | yes | manual CLI (--outdir); make studio-generate-fault-artifacts | Explicit `--report <legacy-json>` when provided; otherwise scans `.repo_studios/command_center/reports/rawview/fault_diagnostics_runs/<ts>/stacks.log`; `utilities.fault_run_analysis` | tests/tests_consumers/test_generate_fault_artifacts.py | manual; Make target; orchestrate_health_suite | scripts/consumers/generate_fault_artifacts.py | std-global-python-engineering.md | none | No longer depends on producer `latest_*` pointers; continues to support scan-mode fallback and writes consumer bundles to `.repo_studios/reports/consumer_reports/fault_artifacts/` with retention controls (`--artifacts-to-keep`) and configurable logging. Catalog: scripts.health.generate_fault_artifacts. |
| .repo_studios/scripts/consumers/generate_test_log_health_report.py | Summarizes pytest logs for warnings, slow tests, and JUnit stats. | yes | manual CLI (--logs-dir/--output-base) | `collect_test_log_reports.py` bundle (`.repo_studios/command_center/reports/rawview/test_log_reports/<YYYYMMDD-HHMM>/telemetry.json`) with fallback to raw pytest log directories; defusedxml | tests/tests_consumers/test_generate_test_log_health_report.py | manual; orchestrate_health_suite | scripts/consumers/generate_test_log_health_report.py | std-global-python-engineering.md | none | Writes JSON/MD/CSV bundles with bundle summaries under `.repo_studios/reports/consumer_reports/test_log_health_reports/<ts>/`, appends source references, computes pass-rate deltas vs. the previous run, defaults retention to 5 (`--artifacts-to-keep`), and surfaces configurable logging while preferring structured producer artifacts with raw log fallback. Catalog: scripts.health.test_log_health_report. |

#!/usr/bin/env python3
"""Duplicate scanning utilities tailored for the library integration workflow.
...
DEFAULT_MIN_LINES = 3
DEFAULT_KEEP_RUNS = 3
DEFAULT_TARGET_RELATIVE = Path(".repo_studios/scripts/producers")
DEFAULT_RUN_ROOT_RELATIVE = Path(".repo_studios/library_integration/reports")
...
def generate_summary(
    ...
    lines = ["# Duplicate Scan Summary", ""]
    lines.append("## Overview")
    lines.append("")
    lines.append(f"- Target directory: `{_to_repo_relative(paths.target, paths.repo_root)}`")
    lines.append(
        f"- Python files scanned: {scan_result.files_scanned}" if scan_result.files_scanned else "- Python files scanned: 0"
    )
    lines.append(f"- Functions analysed: {scan_result.functions_scanned}")
    lines.append(f"- Scanner groups detected: {stats['scanner_groups']}")
    lines.append(f"- Producer groups referenced: {stats['producer_groups']}")
    lines.append(f"- Scanner groups matched to producers: {stats['matched_groups']}")
    lines.append(f"- Scanner-only groups: {stats['scanner_only_groups']}")
    lines.append("")
    lines.append("## Inputs")
    lines.append("")
    rel_analysis = _to_repo_relative(analysis_path, paths.repo_root)
    lines.append(f"- Analysis dataset: `{rel_analysis}`")
    lines.append("- Run generated with scan_duplicates CLI")
    lines.append("")
    lines.append("## Top Duplicate Offenders")
    lines.append("")
    ...
    lines.append("")
    lines.append("- Review scanner-only groups to decide whether they warrant new producer tracking.")
    lines.append("- Prioritise groups with high duplicate counts or similarity for extraction.")
    return "\n".join(lines) + "\n"

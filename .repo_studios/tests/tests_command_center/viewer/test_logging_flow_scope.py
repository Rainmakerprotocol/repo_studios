...
MODULE_PATH = REPO_STUDIOS_ROOT / "command_center" / "viewer" / "ui" / "builders" / "logging_flow_scope.js"

if not MODULE_PATH.exists():  # pragma: no cover - guard against missing assets
    raise AssertionError(f"Expected logging flow scope module at {MODULE_PATH}")
...
def test_scope_filters_to_selected_domain() -> None:
    script = f"""
import {{ resolveLoggingFlowScope }} from "{MODULE_PATH.as_uri()}";
const functions = new Map([
  ["alpha::f1", {{ moduleId: "alpha.service.module1", loggingCalls: [{{ level: "info" }}] }}],
  ["alpha::f2", {{ moduleId: "alpha.service.module2", loggingCalls: [{{ level: "error" }}] }}],
  ["beta::g1", {{ moduleId: "beta.core.module", loggingCalls: [{{ level: "debug" }}] }}],
]);

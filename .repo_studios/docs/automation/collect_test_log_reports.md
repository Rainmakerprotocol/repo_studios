...
- `--logs-dir`: Base directory containing pytest log runs (default `.repo_studios/pytest_logs`).
...
- Ensure `orchestrators/run_pytest_log_capture.py` (or equivalent) populates `.repo_studios/pytest_logs/<slug>/<timestamp>/` prior to running the producer. Missing runs result in a no-op with an informational log entry.

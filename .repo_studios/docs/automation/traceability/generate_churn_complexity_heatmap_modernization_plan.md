...
- **Inputs:** ad-hoc `git log` queries, direct AST walks over the repo tree, and raw `junit_*.xml` files under `.repo_studios/command_center/reports/rawview/test_execution_runs/` (with optional legacy fallback during cutover).

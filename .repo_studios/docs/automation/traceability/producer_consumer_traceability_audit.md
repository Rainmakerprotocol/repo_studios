...
| `.repo_studios/scripts/producers/generate_anchor_inventory.py` → `.repo_studios/reports/producer_reports/anchor_inventory_reports/` (`report.json`, `latest_report.json`) | `.repo_studios/scripts/consumers/generate_anchor_health_report.py` → `summary.json`, `SUMMARY.md`, `bundle_summary.json`, legacy `anchor_report.json`, `anchor_report.md`, `clusters.tsv`, and `runs.log`, plus latest hardlinks | Structured summary now captures source, baseline deltas, and top clusters with Source References, while legacy report files remain for compatibility; run log continues to track execution timestamps. | Default retention keeps 10 timestamped runs via `--artifacts-to-keep`; full fallback docs scan is retained as `source="scan"` when inventories are missing. |

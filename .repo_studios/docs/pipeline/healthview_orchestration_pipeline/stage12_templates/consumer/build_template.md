...
**Consumer (Tier A Report Generator) — REQUIRED keys:**
...
| `upstream_refs` | list | ✅ | List of upstream artifact paths consumed |
...
#### 2.4.2 HOP Bundle Compliance (Tier A — Consumer)
...
| <upstream artifact loaded> | Check logs for "loaded" vs "not found" | <loaded/skipped> | ✅/❌ |
...
compliance_tier: A
entry_point: run
description: "<One-line description of what upstream data this consumer processes>"
version: "1.0.0"
...
inputs:
  ...
  - name: upstream_dir
    type: path
    required: false
    description: "Directory containing upstream producer output"
  - name: log_level
    type: choice
...
outputs:
  ...
  upstream_refs: "List of upstream artifact paths consumed"
  # <additional outputs>
...
**For Consumer (Tier A Report Generator):**

| Artifact | Target Table | Key Columns |
|----------|--------------|-------------|
| manifest.json | `hop_manifests` | viewer_slug, topic, run_timestamp, schema_version, upstream_refs |
...
#### 3.1.2 HOP Bundle Gaps (Consumer)
...
| Missing upstream artifact resolution | High | M |
...
|----------|-------------|
| `<SCRIPT_NAME>` | Script filename (e.g., `process_health_report.py`) |
| `<SCRIPT_PATH>` | Full path (e.g., `.repo_studios/scripts/consumers/process_health_report.py`) |
| `<SCRIPT_DIR>` | Script directory (e.g., `.repo_studios/scripts/consumers`) |
| `<RECORD_ID>` | ASR record ID (e.g., `ASR-008`) |
| `<YYYY-MM-DD>` | ISO date |
| `<LINE_COUNT>` | Script line count |
| `<TARGET_STAGE>` | Destination stage (e.g., `Stage 4.2`) |
| `<TOPIC>` | Topic slug (e.g., `health_processing`) |

...
<Brief description of what final digest this summarizer produces from upstream aggregator outputs>

### 1.2 Current Capabilities

- Reads: <aggregator output(s)>
- Produces: <final digest description>
...
| `--aggregator-dir` | path | auto | Directory containing upstream aggregator output |
...
**Summarizer (Tier A Report Generator) — REQUIRED keys:**
...
| `aggregator_refs` | list | ✅ | List of aggregator artifacts consumed |

### 2.3 Upstream Dependencies

| Aggregator | Artifact | Description |
|------------|----------|-------------|
| `<aggregator_name>` | `<artifact_path>` | <what data is consumed> |
...
|----------|--------|-------------|
| `manifest.json` | JSON | Schema version, status, inputs, aggregator refs |
| `summary.md` | Markdown | Human-readable final digest |
...
#### 2.4.2 HOP Bundle Compliance (Tier A — Summarizer)
...
| Aggregator artifact resolution | ⚠️/✅ | <evidence> |
...
| <aggregator data loaded> | Check logs for "loaded" vs "not found" | <loaded/skipped> | ✅/❌ |
| <final digest metrics accurate> | Cross-reference with aggregator outputs | <actual metrics> | ✅/❌ |
...
compliance_tier: A
entry_point: run
description: "<One-line description of what final digest this summarizer produces>"
version: "1.0.0"
...
inputs:
  ...
  - name: aggregator_dir
    type: path
    required: false
    description: "Directory containing upstream aggregator output"
  - name: log_level
    type: choice
...
outputs:
  ...
  aggregator_refs: "List of aggregator artifacts consumed"
  # <additional outputs>
...
upstream_dependencies:
  - aggregator: <upstream_aggregator>
...
**For Summarizer (Tier A Report Generator):**

| Artifact | Target Table | Key Columns |
|----------|--------------|-------------|
| manifest.json | `hop_manifests` | viewer_slug, topic, run_timestamp, schema_version, aggregator_refs |
...
# DB_INTEGRATION_MARKER: hop_summaries.content_md — Human-readable final digest
...
#### 3.1.2 HOP Bundle Gaps (Summarizer)
...
| Missing aggregator artifact resolution | High | M |
...
|----------|-------------|
| `<SCRIPT_NAME>` | Script filename (e.g., `create_final_digest.py`) |
| `<SCRIPT_PATH>` | Full path (e.g., `.repo_studios/scripts/summarizers/create_final_digest.py`) |
...
| `<TOPIC>` | Topic slug (e.g., `final_digest`) |

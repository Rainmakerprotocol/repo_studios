...
related_files:
  ...
  - .repo_studios/docs/pipeline/healthview_orchestration_pipeline/tier3_scripts/available_scripts_oversight/tier3_check_inventory_health.yaml
...
| **Tier-3 YAML** | `tier3_scripts/available_scripts_oversight/tier3_check_inventory_health.yaml` |
...
ScriptConfig(
    asr_id="ASR-007",
    script_name="check_inventory_health",
    module_path=".repo_studios.command_center.scripts.producers.generate_inventory_health.check_inventory_health",
    description="Inventory health checker",
    default_args=["--repo-root", "."],
    supports_output_dir=False,  # CRITICAL: Script uses create_storage() for HOP-compliant paths
)
```

> **⚠️ SAFETY WARNING — supports_output_dir**
>
> This script uses `create_storage()` which builds topic-aware output paths.
...
> pruning incidents. The `False` setting preserves the script's HOP-compliant
...
|------|--------|---------|--------|
| 2026-01-28 | Agent | Formalized from temp doc; added Tier-3 YAML reference; added Section 6.3 Orchestrator Integration with safety warning | Current |

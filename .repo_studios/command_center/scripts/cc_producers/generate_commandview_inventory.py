...
try:
    ...
    from libraries import resolve_repo_root, slugify_relative  # noqa: E402
...
def _first_stmt_kind(node: ast.AST) -> str | None:
    ...
    if not node.body:
        return None
    return node.body[0].__class__.__name__.lower()
...
def _extract_function(
    ...
    if end_lineno is not None and start_lineno is not None:
        line_count = max(0, end_lineno - start_lineno + 1)
...
def _resolve_call_target(
    ...
    if caller_class and (binding_root in {"self", "cls"} or call.get("is_method_like")) and attr_tail_parts:
        ...
        method_target = class_methods.get(caller_class, {}).get(method_candidate)
        if method_target:
            method_detail: dict[str, Any] | None = None
            if len(attr_tail_parts) > 1:
                method_detail = {"attribute_tail": ".".join(attr_tail_parts[1:])}
            return _finalize(
                method_target,
                "local_method",
                module_id,
                "medium" if method_detail else "high",
                method_detail,
            )

    if root and root in class_methods and attr_tail_parts:
        method_candidate = attr_tail_parts[0]
        root_method_target = class_methods[root].get(method_candidate)
        if root_method_target:
            root_method_detail: dict[str, Any] | None = None
            if len(attr_tail_parts) > 1:
                root_method_detail = {"attribute_tail": ".".join(attr_tail_parts[1:])}
            return _finalize(
                root_method_target,
                "local_method",
                module_id,
                "medium" if root_method_detail else "high",
                root_method_detail,
            )
    ...
    if module_basename and root == module_basename and attr_tail_parts:
        ...
        if candidate in local_symbols:
            module_detail: dict[str, Any] | None = None
            if len(attr_tail_parts) > 1:
                module_detail = {"attribute_tail": ".".join(attr_tail_parts[1:])}
            return _finalize(
                local_symbols[candidate],
                "local_function",
                module_id,
                "medium" if module_detail else "high",
                module_detail,
                {"via_module": module_basename},
            )
        if candidate in class_methods and len(attr_tail_parts) > 1:
            method_candidate = attr_tail_parts[1]
            module_method_target = class_methods[candidate].get(method_candidate)
            if module_method_target:
                module_method_detail: dict[str, Any] | None = None
                if len(attr_tail_parts) > 2:
                    module_method_detail = {"attribute_tail": ".".join(attr_tail_parts[2:])}
                return _finalize(
                    module_method_target,
                    "local_method",
                    module_id,
                    "medium" if module_method_detail else "high",
                    module_method_detail,
                    {"via_module": module_basename},
                )
    ...
    if binding_expression and binding_expression != binding_root:
        resolution_detail["binding_expression"] = binding_expression
    if attr_tail_parts:
        resolution_detail.setdefault("attribute_tail", ".".join(attr_tail_parts))
    if binding_root:
        resolution_detail.setdefault("binding", binding_root)
...
def _summarize_dependency_categories(graph: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for category, modules_set in modules_by_category.items():
        if category in DEPENDENCY_SUMMARY_BUCKETS:
            continue
        summary[category] = {
            "count": len(modules_set),
...
def analyze_python_file(
    ...
    for walk_node in ast.walk(tree):
        if isinstance(walk_node, ast.Call):
            callee = _safe_unparse(walk_node.func) or ""
            if "ArgumentParser" in callee or (
                isinstance(walk_node.func, ast.Attribute) and walk_node.func.attr == "add_argument"
            ):
                cli_parser = True
...
def _resolve_callee_path(
    ...
    resolved: str | None
    if remainder:
        resolved = f"{target}.{remainder}" if target else remainder
    else:
        resolved = target

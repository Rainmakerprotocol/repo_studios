...
<!-- Technical documentation with inline code examples; line length exempt -->
...
1. **Query:** "Which files have high complexity AND high churn?"
   - **Tables:** `test_metrics`
   - **Key fields:** `churn_stats` (JSONB)
   - **SQL:** See Q3 in Time-Series Considerations

1. **Query:** "Show me all runs where tests failed"
   - **Tables:** `report_runs`, `test_metrics`
   ...
   - **SQL:**
     ...
     FROM report_runs rr
     JOIN test_metrics tm ON rr.id = rr.run_id
     WHERE rr.topic = 'test_execution_telemetry'

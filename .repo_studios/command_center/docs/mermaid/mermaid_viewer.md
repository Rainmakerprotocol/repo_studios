...
- **State Effects Pack**
  - `global_variable_usage_map.mmd`
    ...
    - [ ] Multi-view coexistence verified (M) — Pending once additional State Effects diagrams (IO Effects, Exception Flow) land so a dedicated coexistence harness can cover cross-view toggles.
    ...
    - Present: Global Variable Usage Map renders module subgraphs with repository fallbacks; Node-backed regressions `test_global_variable_usage_map_view.py` and `test_global_variable_usage_view_definition.py` lock deterministic output and scope handling.
    - Future: Wire the Exception Flow map next, then extend the State Effects coexistence suite to cover both views.
  - `io_effects_diagram.mmd`
    ...
    - [ ] Multi-view coexistence verified (M) — Pending Exception Flow wiring so a dedicated State Effects harness can exercise both diagrams.
    ...
    - Present: IO Effects Diagram highlights per-module side effects with stats/fallbacks guarded by Node-backed regressions `test_io_effects_diagram_view.py` and `test_io_effects_view_definition.py` (2025-11-12, GitHub Copilot).
    - Future: Add effect severity overlays once producers emit richer metadata and include the view in the State Effects coexistence suite alongside Exception Flow.

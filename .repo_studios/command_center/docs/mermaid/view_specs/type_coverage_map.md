...
3. Bucket each function into `strong` (≥ 0.8), `moderate` (0.5–0.79), `weak` (< 0.5), or `unknown` when coverage cannot be parsed.
...
5. Generate Mermaid nodes for the central hub plus each bucket, applying class definitions to keep colors consistent with the Quality Metrics palette.
...
- Builder: `buildTypeCoverageMapDiagram()` in `.repo_studios/command_center/viewer/ui/builders/type_coverage_map.js`, consumed by `buildTypeCoverageMapViewDefinition()` in `.repo_studios/command_center/viewer/ui/viewer.js`.
...
- Introduce optional module scoping so operators can filter buckets by selected module for deeper analysis.

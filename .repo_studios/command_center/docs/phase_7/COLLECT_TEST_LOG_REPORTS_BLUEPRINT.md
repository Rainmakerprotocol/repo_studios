...
- Orchestrators capture pytest runs into `.repo_studios/command_center/reports/rawview/test_execution_runs/<slug>/<timestamp>/` directories (legacy `.repo_studios/pytest_logs/` runs remain available during the migration window).
...
1. `--logs-dir`: Base directory (default `.repo_studios/command_center/reports/rawview/test_execution_runs`; legacy `.repo_studios/pytest_logs` discovery remains available while the migration flag `PYTEST_LOG_REPORTS_ALLOW_LEGACY` is enabled).

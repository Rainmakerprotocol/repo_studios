...
- Latest lizard complexity report under `.repo_studios/reports/producer_reports/healthview/lizard_report/<YYYYMMDD-HHMM>/`. The `summary.md` file surfaces the top offenders with path, line numbers, delta over thresholds, and recommended actions.
...
1. **Review Top Offenders**  
   Open the newest `summary.md` in `.repo_studios/reports/producer_reports/healthview/lizard_report/<YYYYMMDD-HHMM>/`. Note any functions highlighted in orchestrator scripts or shared helpers. Confirm whether they overlap with the duplicate groups scheduled for extraction.

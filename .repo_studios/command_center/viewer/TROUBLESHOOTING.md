...
1. **Open the diagnostic page**:
   <http://127.0.0.1:8000/.repo_studios/command_center/viewer/ui/diagnostic.html>
1. **Click "Run All Tests"** to verify configuration and network connectivity
1. **Check browser console** (F12) for detailed logging
...
1. Look for `[refreshSelectorData]` logs
1. Check if selector.json is being fetched successfully
...
1. Console for `[loadCommandViewPayloads]` logs
1. Network tab for artifact JSON files (should be 200 OK)
...
1. Check if data requirements are met
...
1. Check console for requirement errors
1. Verify artifact has necessary data fields
...
1. ✓ `viewer.css`
1. ✓ `viewer.js`
1. ✓ `mermaid.min.js` (from CDN)
1. ✓ `selector.json`
1. ✓ `producers_commandview_*.json` (when artifact clicked)
1. ✓ `producers_commandview_screening_*.json` (optional)
...
1. **Capture network errors**: Screenshot Network tab showing any 404s
1. **Check diagnostic results**: Screenshot diagnostic.html test results

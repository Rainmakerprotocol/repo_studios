...
| `.repo_studios/scripts/orchestrators/run_standards_index_cli.py` | Standards Integrity orchestrator plus ongoing CLI shim (`run_standards_index_cli.py`) for ad-hoc queries | The CLI still supports `list/search/show/stats` with `--severity`, `--category`, and `--artifacts-to-keep`; Standards Integrity now captures the same catalog data inside its manifest and summary, and automation should invoke the topic runner directly (for example `make -C .repo_studios studio-orchestrate-standards`) while the shim handles interactive lookups. | CLI artifacts formerly landed in `.repo_studios/reports/orchestrator_runs/standards_index_cli/` (retired 2025-12-10); the topic orchestrator mirrors the same catalog data inside the Healthview bundle so dashboards can consume the new payload before the shim disappears. | CLI retention relied on `--artifacts-to-keep 5`; the Standards Integrity runner centralises retention with helper-managed keep counts (three manifests by default, producer-specific keeps for index/diff/prompt). |

### Artifact Migration Status (2025-12-10)

- Retired the legacy `.repo_studios/reports/orchestrator_runs/*` and `.repo_studios/reports/orchestrator_logs/*` directories. Pytest log capture runs now publish under `.repo_studios/command_center/reports/rawview/test_execution_runs/` alongside the structured collector bundles. Batch cleanup dry-run plans now publish under `.repo_studios/command_center/reports/rawview/dependency_import_hygiene_cleanup/`, and refreshed mypy baselines live in `.repo_studios/command_center/reports/rawview/mypy_baselines/` alongside existing `latest_*` pointers for downstream agents.
...
- [x] Phase 7 – Validation and Rollout — Completed 2025-12-10 after backfilling skip handling for empty mypy targets, rerunning the Dependency & Import Hygiene topic pipeline, and confirming Healthview bundle `command_center/reports/healthview/dependency_import_hygiene/20251207-0330/` reports a successful run with `typecheck_status=skipped` and zero findings.
  - [x] Run sequential dry runs across all topics, recording runtimes, artifact paths, and
    ...
    - 2025-12-10: Re-ran the Dependency & Import Hygiene orchestrator via
      `C:/Users/genet/repo_studios/.venv/Scripts/python.exe -m command_center.scripts.orchestrators.run_dependency_import_hygiene --repo-root C:/Users/genet/repo_studios --log-level INFO` (with `PYTHONPATH` set to `.repo_studios`) and inspected
      `.repo_studios/command_center/reports/healthview/dependency_import_hygiene/20251207-0330/summary.md`,
      verifying the pipeline now records `typecheck_status: skipped` alongside the new skip note and
      no failing telemetry steps.
    - 2025-12-10: Validated the producer/test coverage fix with
      `.venv/Scripts/python.exe -m pytest .repo_studios/tests/tests_producers/test_generate_typecheck_report.py` (exercising the new
      `test_typecheck_skips_when_no_targets` and `test_typecheck_missing_target_output_is_skipped` cases) plus the orchestrator suites
      (`.repo_studios/tests/tests_command_center/dependency_import_hygiene` and
      `.repo_studios/tests/tests_command_center/orchestrators/test_orchestrate_full_diagnostic.py`) to maintain ≥80% coverage for every touched module.
...
- [x] 2025-12-10: Updated `.repo_studios/scripts/producers/generate_typecheck_report.py` to
  normalise empty mypy targets and downgrade "Missing target module" output to a documented skip,
  added regression coverage in `.repo_studios/tests/tests_producers/test_generate_typecheck_report.py`,
  and revalidated orchestrator telemetry via
  `.venv/Scripts/python.exe -m pytest .repo_studios/tests/tests_command_center/dependency_import_hygiene`
  plus `C:/Users/genet/repo_studios/.venv/Scripts/python.exe -m command_center.scripts.orchestrators.run_dependency_import_hygiene --repo-root C:/Users/genet/repo_studios --log-level INFO`
  (with `PYTHONPATH` set to `.repo_studios`), confirming Healthview bundle
  `command_center/reports/healthview/dependency_import_hygiene/20251207-0330/manifest.json` records a
  successful pipeline and skipped typecheck step.

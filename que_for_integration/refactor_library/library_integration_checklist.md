...
- [x] **Seed run workspace** by copying the current checklist into `.repo_studios/library_integration/checklists/<YYYYMMDD>.md` and recording baseline notes *(Agent → initial copy, Developer → verify).* — Snapshot `checklists/2025-10-24.md` captured prior to implementation kickoff.
...
- [x] **Define report retention** in `.repo_studios/library_integration/reports/` (naming, pruning cadence) *(Agent → proposal, Developer → approve).* — Default keep count set to three timestamped runs with `.keep` override respected via `scan_duplicates.py` retention helper.
...
- [x] **Create target module scaffold** under `.repo_studios/scripts/library_integration/duplicates/` with `__init__.py`, a refactored `scan_duplicates.py` module (pure functions), and a thin CLI entrypoint mirroring `generate_function_analysis.py` patterns.
- [x] **Relocate and adapt tests** into `.repo_studios/tests/tests_library_integration/duplicates/test_scan_duplicates.py`, updating imports to the new module path and parameterizing fixtures for Windows and POSIX paths.
...
- [x] **Document usage** in `.repo_studios/library_integration/README.md` and add a runbook snippet to `docs/duplicate_detection_schema_alignment.md` covering invocation examples and expected outputs.
...
- Naming conventions from `.repo_studios/library_integration/docs/naming_conventions.md` (training-mode copy of canonical rules) establish the library structure; no alternative hierarchy approved yet.
